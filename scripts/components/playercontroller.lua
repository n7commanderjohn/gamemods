...
function PlayerController:OnEquip(data)
	...
	if data.eslot == EQUIPSLOTS.HANDS or data.eslot == BOATEQUIPSLOTS.BOAT_LAMP then

        local newreticule = data.item.components.reticule or self.inst.components.reticule
        if newreticule ~= self.reticule then
            ...
            if newreticule ~= nil and newreticule.reticule == nil and (newreticule.mouseenabled or controller_mode) then
...
function PlayerController:OnControl(control, down)
	...
	if not IsPaused() then
		...
		if GetPlayer() and (control == CONTROL_SECONDARY or control == CONTROL_CONTROLLER_ALTACTION) then
			if not down then
		...
		if down then
			if self.placer_recipe and control == CONTROL_CANCEL then
				...
				if inv_obj then
					if inv_obj.onshelf then
						-- shelved items behave a bit different, they get their action from the slot they're in
						if control == CONTROL_INVENTORY_USEONSCENE then
							local l, r = self:GetSceneItemControllerAction(inv_obj.onshelf)
							local action = l or r
							self:DoAction(action)
						end
					else
						local is_equipped = (inv_obj.components.equippable and inv_obj.components.equippable:IsEquipped())
						local un_unequipable = (inv_obj.components.equippable and inv_obj.components.equippable.un_unequipable)
						...
						elseif control == CONTROL_INVENTORY_USEONSELF and is_equipped and not un_unequipable then
						...
						elseif control == CONTROL_INVENTORY_USEONSELF and not is_equipped then
							if inv_obj.components.deployable and inv_obj.components.deployable:IsDeployable(GetPlayer()) and not self.deploy_mode and inv_obj.components.inventoryitem:GetGrandOwner() == self.inst then
						...
						elseif control == CONTROL_INVENTORY_USEONSCENE and is_equipped and not un_unequipable then
...
function PlayerController:DoControllerAction()
	...
	if self.placer then
		if self.placer.components.placer.can_build then
			...
			if self.placer.components.placer.modifyfn then
				modifydata = self.placer.components.placer.modifyfn(self.placer)
			end
...
function PlayerController:DoControllerAltAction()
	...
	r = self:GetGroundUseSpecialAction(nil, true)
	if r then
		self:DoAction(r)
		return
	end
...
function PlayerController:DoFlipFacing(dir)
	if self.placer_recipe and self.placer_recipe.flipable then
		if self.placer then
			if self.placer.flipsrotate then
				...
				local rotation = self.placer.Transform:GetRotation()
				self.placer.Transform:SetRotation(rotation * -1)
			end
		end
...
function PlayerController:GetHoverTextOverride()
	if self.placer_recipe then
		local string = STRINGS.UI.HUD.BUILD.. " " .. ( STRINGS.NAMES[string.upper(self.placer_recipe.name)] or STRINGS.UI.HUD.HERE )
...
function PlayerController:GetControllerHoverTextOverride(controller_id)
	if self.placer_recipe then
		local string = STRINGS.UI.HUD.BUILD.. " " .. ( STRINGS.NAMES[string.upper(self.placer_recipe.name)] or STRINGS.UI.HUD.HERE )
		if self.placer_recipe.flipable then
			local lb = TheInput:GetLocalizedControl(controller_id, CONTROL_ROTATE_LEFT)
			local rb = TheInput:GetLocalizedControl(controller_id, CONTROL_ROTATE_RIGHT)
			string = string.."\n"..TheInput:GetLocalizedControl(controller_id, CONTROL_CONTROLLER_ACTION) .. " " .. STRINGS.UI.HUD.BUILD.."\n" .. TheInput:GetLocalizedControl(controller_id, CONTROL_CONTROLLER_ALTACTION) .. " " .. STRINGS.UI.HUD.CANCEL
			string = string.. "\n" .. lb..rb.." "..STRINGS.UI.HUD.FLIP_CONTROLLER
		end
		return string
	end
end
				...
				if (guy:HasTag("monster") and has_weapon) or
					...
					force_attack then
						...
						if guy:HasTag("spider") and self.inst:HasTag("spiderwhisperer") then
							ok = false
							if guy.components.combat and guy.components.combat.target and (guy.components.combat.target == self.inst or (guy.components.combat.target.components.follower and guy.components.combat.target.components.follower.leader == self.inst)) then							
								ok = true
						    end
						end
...
function PlayerController:UpdateControllerAttackTarget(dt)
	...
	local rad = 8
	if self.controller_attack_target and self.controller_attack_target:IsValid() and self:CanAttackWithController(self.controller_attack_target) and not self.controller_attack_target:HasTag("lastresort") then
		...
		if canattack then
			...
			if (dist < min_rad or dot > 0) and dist < max_range then
				...
				if v:HasTag("lastresort") then
					lastresort = v
				elseif not target or target_score < score then
					target = v
					target_score = score
				end
			end
...
function PlayerController:UpdateControllerReticule()
	...
	if not equipped_item or not equipped_item.components.reticule then
		equipped_item = boatitem
	end

	local newreticule = equipped_item ~= nil and equipped_item.components.reticule or self.inst.components.reticule
	if newreticule ~= self.reticule then
	    if self.reticule ~= nil then
	        self.reticule:DestroyReticule()
	        self.reticule = nil
	    ...
	    if newreticule ~= nil and newreticule.reticule == nil then
	    	if (newreticule:NeedsValidation() and newreticule:Validate()) or not newreticule:NeedsValidation() then
	    		self.reticule = newreticule
	    		newreticule:CreateReticule()
	        end
	    end
	elseif self.reticule ~= nil and self.reticule:NeedsValidation() and not self.reticule:Validate() then
		self.reticule:DestroyReticule()
		self.reticule = nil
	end
end
...
function PlayerController:DoAction(buffaction)
    if buffaction then
        ...
        if  buffaction.invobject and 
            ...
            (buffaction.action ~= ACTIONS.DROP and buffaction.action ~= ACTIONS.STORE and buffaction.action ~= ACTIONS.FERTILIZE) then
...
function PlayerController:OnLeftClick(down)
	...
	if self.placer_recipe and self.placer then
		...
		if self.placer.components.placer.can_build then
			...
			self.inst.components.builder:MakeRecipe(self.placer_recipe, pos, self.placer:GetRotation(), nil, modifydata)
			self:CancelPlacement()
		end
		return		
	end
...
function PlayerController:GetGroundUseAction()
	...
	if TheInput:ControllerAttached() and isBoating then --Does position == nil mean controller anyway? 
        ...
        for i = 0, numSteps, 1 do 
            local testPos = myPos + dir * step * i 
            local testTile = GetWorld().Map:GetTileAtPoint(testPos.x , testPos.y, testPos.z) 
            if not GetWorld().Map:IsWater(testTile) and testTile ~= GROUND.IMPASSABLE and testTile ~= GROUND.INVALID then 
...
function PlayerController:GetItemUseAction(active_item, target)
    ...
    if target then
		-- if this is a shelfed item, query the shelf instead
		target = target.onshelf or target
